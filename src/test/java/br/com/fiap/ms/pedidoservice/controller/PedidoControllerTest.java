...
class PedidoControllerTest {
    ...
    void deveBuscarPedidosComSucesso() {
        List<PedidoResponseJson> listaPedidos = List.of(
                new PedidoResponseJson(UUID.randomUUID(), 123, "33344455566",
                        new BigDecimal("30.0"), UUID.randomUUID(), LocalDateTime.now(), null,
                        StatusPedido.ABERTO, null)
        );
    ...
    void deveExcluirPedidoComSucesso() {
        ...
        doNothing().when(excluir).excluirPedido(id);

        ResponseEntity<String> response = controller.excluirPedido(id);

        assertEquals(204, response.getStatusCodeValue());
        assertEquals(null, response.getBody());
        verify(excluir, times(1)).excluirPedido(id);
    }

    @Test
    void deveRetornarNotFoundAoExcluirPedidoInexistente() {
        UUID id = UUID.randomUUID();

        doThrow(new RuntimeException("Pedido não encontrado")).when(excluir).excluirPedido(id);
